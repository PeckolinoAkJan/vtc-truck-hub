...
public partial class MainWindow:Window
{
    ...
    async Task ProcessPacket(TelemetryPacket p)
    {
        ...
        if(result.Ok){syncedCount++;if(!string.IsNullOrWhiteSpace(result.TripId)){trip.Id=result.TripId;trip.JobKey=jobKey;}tripPoints=Math.Max(tripPoints,result.PointsTotal);if(result.Lifecycle is "confirmed" or "approved"){await Dispatcher.InvokeAsync(CompleteTrip);}else if(result.Lifecycle=="pending_driver"){await Dispatcher.InvokeAsync(CompleteTrip);await ConfirmInvoiceAsync();}else if(result.Lifecycle=="cancelled")await Dispatcher.InvokeAsync(AbortTrip);else if(result.Lifecycle=="interrupted"){trip.Status="Unterbrochen";trip.Active=true;}Save(RecoveryPath,trip);Dispatcher.Invoke(()=>{MetricSynced.Text=syncedCount.ToString();PointCounterText.Text=$"{tripPoints} PUNKTE";UpdateTripUi();});}
