...
public partial class MainWindow:Window
{
    ...
    readonly Button[] navButtons; DateTime lastSent=DateTime.MinValue,lastFleetWarning=DateTime.MinValue; string lastFleetReason="";

    public MainWindow()
    {
        InitializeComponent(); Directory.CreateDirectory(dataDir); settings=Load<ClientSettings>(SettingsPath)??new();settings.ApiUrl=NormalizeApiUrl(settings.ApiUrl);settings.ApiKey=Unprotect(settings.ProtectedApiKey);
    ...
    static string NormalizeApiUrl(string? value){if(Uri.TryCreate(value,UriKind.Absolute,out var uri)&&uri.Host.Equals("vtc-truck-hub.de",StringComparison.OrdinalIgnoreCase))return "https://vtc-truck-hub.de";if(Uri.TryCreate(value,UriKind.Absolute,out uri)&&uri.Scheme==Uri.UriSchemeHttps)return uri.GetLeftPart(UriPartial.Authority);return "https://vtc-truck-hub.de";}
    string Api(string path)=>NormalizeApiUrl(settings.ApiUrl).TrimEnd('/')+path;
    ...
    async Task ProcessPacket(TelemetryPacket p)
    {
        ...
        var mapPosition=ScsCoordinateProjector.ProjectWithFallback(p.Game,p.WorldX,p.WorldZ);
        var payload=new{tripId=trip.Id,jobKey,hasJob,@event=p.Event??"telemetry",vtcId=SelectedVtc(),userId=settings.UserId,game=p.Game is "ATS"?"ATS":"ETS2",latitude=mapPosition.Latitude,longitude=mapPosition.Longitude,gameX=p.WorldX,gameY=p.WorldY,gameZ=p.WorldZ,coordinateAccuracy=mapPosition.Accuracy,projectionProfile=mapPosition.Profile,heading=p.Heading,speedKph=p.SpeedKph,rpm=p.Rpm,gear=p.Gear,fuelLiters=p.Fuel,fuelAverage=p.FuelAverage,fuelRange=p.FuelRange,odometerKm=p.Odometer,truckDamage=p.TruckDamage,trailerDamage=p.TrailerDamage,cargoDamage=p.CargoDamage,cruiseControl=p.CruiseControl,engineEnabled=p.EngineEnabled,parkingBrake=p.ParkingBrake,motorBrake=p.MotorBrake,retarderLevel=p.RetarderLevel,leftBlinker=p.LeftBlinker,rightBlinker=p.RightBlinker,hazardWarning=p.HazardWarning,lowBeam=p.LowBeam,highBeam=p.HighBeam,beacon=p.Beacon,brakeAirPressure=p.BrakeAirPressure,waterTemperature=p.WaterTemperature,batteryVoltage=p.BatteryVoltage,steeringInput=p.SteeringInput,throttleInput=p.ThrottleInput,brakeInput=p.BrakeInput,navigationDistance=p.NavigationDistance,navigationTime=p.NavigationTime,navigationSpeedLimitKph=p.NavigationSpeedLimitKph,gameTime=p.GameTime,cargo=p.Cargo,cargoMass=p.CargoMass,sourceCity=p.SourceCity,sourceCompany=p.SourceCompany,destinationCity=p.DestinationCity,destinationCompany=p.DestinationCompany,plannedDistanceKm=p.PlannedDistanceKm,gameIncomeCents=p.GameIncomeCents,truck=p.Truck,truckAsset=new{configId=p.TruckConfigId,brandId=p.TruckBrandId,brand=p.TruckBrand,name=p.TruckName,licensePlate=p.TruckLicensePlate,licenseCountryId=p.TruckLicenseCountryId},trailerAsset=p.HasTrailer?new{configId=p.TrailerConfigId,brandId=p.TrailerBrandId,brand=p.TrailerBrand,name=p.TrailerName,licensePlate=p.TrailerLicensePlate,licenseCountryId=p.TrailerLicenseCountryId,bodyType=p.TrailerBodyType,chainType=p.TrailerChainType}:null,server=p.Server,recordedAt=DateTime.UtcNow};
        var json=JsonSerializer.Serialize(payload);var result=await Send(json);
        if(result.Ok){syncedCount++;if(!string.IsNullOrWhiteSpace(result.TripId)){trip.Id=result.TripId;trip.JobKey=jobKey;}tripPoints=Math.Max(tripPoints,result.PointsTotal);if(result.Fleet is not null)await Dispatcher.InvokeAsync(()=>ApplyFleetState(result.Fleet));if(result.Lifecycle is "confirmed" or "approved"){await Dispatcher.InvokeAsync(CompleteTrip);}else if(result.Lifecycle=="pending_driver"){await Dispatcher.InvokeAsync(CompleteTrip);await ConfirmInvoiceAsync();}else if(result.Lifecycle=="cancelled")await Dispatcher.InvokeAsync(AbortTrip);else if(result.Lifecycle=="interrupted"){trip.Status="Unterbrochen";trip.Active=true;}else if(result.Lifecycle=="fleet_blocked"){trip.Status="Gesperrt – anderes Fahrzeug oder anderen Auflieger verwenden";trip.Active=true;}else if(result.Lifecycle=="resumed"){trip.Status="Fortgesetzt – Fahrzeugprüfung bestanden";trip.Active=true;}Save(RecoveryPath,trip);Dispatcher.Invoke(()=>{MetricSynced.Text=syncedCount.ToString();PointCounterText.Text=$"{tripPoints} PUNKTE";UpdateTripUi();});}
        else{File.AppendAllText(QueuePath,json+Environment.NewLine);Dispatcher.Invoke(UpdateQueueCount);}
    }
    async Task ConfirmInvoiceAsync(){if(string.IsNullOrWhiteSpace(trip.Id)||string.IsNullOrWhiteSpace(settings.ApiKey))return;try{var json=JsonSerializer.Serialize(new{action="confirmTrip",tripId=trip.Id});using var request=new HttpRequestMessage(HttpMethod.Post,Api("/api/v1/payroll")){Content=new StringContent(json,Encoding.UTF8,"application/json")};request.Headers.Authorization=new AuthenticationHeaderValue("Bearer",settings.ApiKey);var res=await http.SendAsync(request,shutdown.Token);if(!res.IsSuccessStatusCode){var error=await res.Content.ReadAsStringAsync();Log("Automatische Abrechnungsübergabe fehlgeschlagen: "+error);return;}trip.Status="Automatisch übermittelt – wartet auf Lohnfreigabe";Save(RecoveryPath,trip);await Dispatcher.InvokeAsync(UpdateTripUi);Log("Abrechnung automatisch zur Lohnfreigabe eingereicht: "+trip.Id);}catch(Exception ex){Log("Automatische Abrechnungsübergabe fehlgeschlagen: "+ex.Message);}}
    async Task<SendResult> Send(string json){try{using var req=new HttpRequestMessage(HttpMethod.Post,Api("/api/v1/telemetry")){Content=new StringContent(json,Encoding.UTF8,"application/json")};req.Headers.Authorization=new AuthenticationHeaderValue("Bearer",settings.ApiKey);var res=await http.SendAsync(req,shutdown.Token);if(!res.IsSuccessStatusCode)return new(false);var response=await res.Content.ReadAsStringAsync();using var doc=JsonDocument.Parse(response);var root=doc.RootElement;var points=root.TryGetProperty("points",out var p)&&p.TryGetProperty("total",out var total)?total.GetInt32():0;FleetResponse? fleet=null;if(root.TryGetProperty("fleet",out var fleetNode))fleet=JsonSerializer.Deserialize<FleetResponse>(fleetNode.GetRawText(),new JsonSerializerOptions{PropertyNameCaseInsensitive=true});return new(true,root.TryGetProperty("tripId",out var id)?id.GetString():null,root.TryGetProperty("lifecycle",out var lc)?lc.GetString():null,points,fleet);}catch{return new(false);}}
    ...
    void ApplyFleetState(FleetResponse fleet)
    {
        if(fleet.Status=="blocked")
        {
            var reason=string.IsNullOrWhiteSpace(fleet.Reason)?"Dieses Fahrzeug oder dieser Auflieger ist gesperrt.":fleet.Reason!;
            FleetWarningPanel.Visibility=Visibility.Visible;FleetWarningPanel.Background=new SolidColorBrush((Color)ColorConverter.ConvertFromString("#4A2025"));FleetWarningPanel.BorderBrush=Brushes.IndianRed;FleetWarningText.Text="FUHRPARK-SPERRE";FleetWarningDetail.Text=reason+". Bitte nutzen Sie ein anderes Fahrzeug oder einen anderen Auflieger.";AnomalyText.Text="Fahrt wegen Fuhrparkstatus gesperrt";AnomalyText.Foreground=Brushes.IndianRed;
            if(lastFleetReason!=reason||(DateTime.UtcNow-lastFleetWarning).TotalSeconds>=60){lastFleetReason=reason;lastFleetWarning=DateTime.UtcNow;System.Media.SystemSounds.Hand.Play();tray.ShowBalloonTip(6000,"VTC Truck Hub Fuhrparksperre",FleetWarningDetail.Text,Forms.ToolTipIcon.Error);if(settings.VoiceWarnings&&fleet.VoiceWarning)_=SpeakFleetWarning(reason);}
        }
        else if(fleet.Status=="review")
        {
            FleetWarningPanel.Visibility=Visibility.Visible;FleetWarningPanel.Background=new SolidColorBrush((Color)ColorConverter.ConvertFromString("#4A361C"));FleetWarningPanel.BorderBrush=Brushes.Orange;FleetWarningText.Text="FAHRZEUGZUORDNUNG PRÜFEN";FleetWarningDetail.Text=fleet.Reason??"Das erkannte Fahrzeug muss im Fuhrpark gekoppelt werden.";
        }
        else
        {
            FleetWarningPanel.Visibility=Visibility.Collapsed;lastFleetReason="";
        }
    }

    async Task SpeakFleetWarning(string reason)
    {
        await Task.Run(()=>{object? voice=null;try{var type=Type.GetTypeFromProgID("SAPI.SpVoice");if(type is null)return;voice=Activator.CreateInstance(type);if(voice is null)return;dynamic speaker=voice;speaker.Volume=100;speaker.Rate=-1;speaker.Speak($"Achtung. {reason}. Bitte nutzen Sie ein anderes Fahrzeug oder einen anderen Auflieger.",0);}catch(Exception ex){Log("Sprachwarnung nicht verfügbar: "+ex.Message);}finally{if(voice is not null&&Marshal.IsComObject(voice))Marshal.FinalReleaseComObject(voice);}});
    }
    ...
    void SaveSettings_Click(object sender,RoutedEventArgs e){if(!Uri.TryCreate(ApiUrlBox.Text.Trim(),UriKind.Absolute,out var server)||server.Scheme!="https"){MessageBox.Show("Bitte verwende eine sichere HTTPS-Serveradresse.","VTC Truck Hub",MessageBoxButton.OK,MessageBoxImage.Warning);return;}if(VtcCombo.Items.Count>0&&!ApiKeyBox.Password.StartsWith("vth_user_",StringComparison.Ordinal)){MessageBox.Show("Der persönliche Clientschlüssel fehlt. Bitte melde dich erneut an.","VTC Truck Hub",MessageBoxButton.OK,MessageBoxImage.Warning);return;}settings.ApiUrl=NormalizeApiUrl(server.GetLeftPart(UriPartial.Authority));settings.ApiKey=ApiKeyBox.Password;settings.SendIntervalMs=int.TryParse(IntervalBox.Text,out var ms)?Math.Clamp(ms,250,10000):1000;settings.AutoStart=AutoStartCheck.IsChecked==true;settings.MinimizeToTray=MinimizeTrayCheck.IsChecked==true;settings.AutoSync=AutoSyncCheck.IsChecked==true;settings.AutoTrip=AutoTripCheck.IsChecked==true;settings.VoiceWarnings=VoiceWarningCheck.IsChecked==true;Save(SettingsPath,settings);using var key=Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run",true);if(settings.AutoStart)key?.SetValue("VTCTruckHub",$"\"{Environment.ProcessPath}\"");else key?.DeleteValue("VTCTruckHub",false);Log("Einstellungen gespeichert");MessageBox.Show("Einstellungen gespeichert.","VTC Truck Hub");}
