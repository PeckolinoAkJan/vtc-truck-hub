...
public partial class MainWindow:Window
{
    ...
    public MainWindow()
    {
        ...
        if(settings.ApiUrl.Contains("localhost",StringComparison.OrdinalIgnoreCase))settings.ApiUrl="https://vtc-truck-hub.de";if(settings.ApiKey=="demo-client-key")settings.ApiKey="";VtcCombo.Items.Clear();ApiUrlBox.Text=settings.ApiUrl;ApiKeyBox.Password=settings.ApiKey;IntervalBox.Text=settings.SendIntervalMs.ToString();AutoStartCheck.IsChecked=settings.AutoStart;MinimizeTrayCheck.IsChecked=settings.MinimizeToTray;AutoSyncCheck.IsChecked=settings.AutoSync;AutoTripCheck.IsChecked=settings.AutoTrip;
    ...
    string SelectedVtc()=>Dispatcher.CheckAccess()?((VtcCombo.SelectedItem as ComboBoxItem)?.Tag?.ToString())??"":Dispatcher.Invoke(SelectedVtc);
    ...
    void ApplyMemberships(JsonElement response){if(response.TryGetProperty("apiBase",out var apiBase)&&Uri.TryCreate(apiBase.GetString(),UriKind.Absolute,out var uri)){settings.ApiUrl=uri.GetLeftPart(UriPartial.Authority);ApiUrlBox.Text=settings.ApiUrl;}VtcCombo.Items.Clear();if(response.TryGetProperty("memberships",out var memberships))foreach(var membership in memberships.EnumerateArray()){string? id=membership.GetProperty("id").GetString(),name=membership.GetProperty("name").GetString(),tag=membership.GetProperty("tag").GetString();if(!string.IsNullOrWhiteSpace(id))VtcCombo.Items.Add(new ComboBoxItem{Content=$"{name} ({tag})",Tag=id});}if(VtcCombo.Items.Count>0)VtcCombo.SelectedIndex=0;else AccountState.Text="Angemeldet, aber noch keiner Spedition zugeordnet";Save(SettingsPath,settings);}
...
public sealed class ClientSettings{public string ApiUrl{get;set;}="https://vtc-truck-hub.de";public string ApiKey{get;set;}="";public string UserId{get;set;}="";public string DriverName{get;set;}="Fahrer";public int SendIntervalMs{get;set;}=1000;public bool AutoStart{get;set;}public bool MinimizeToTray{get;set;}=true;public bool AutoSync{get;set;}=true;public bool AutoTrip{get;set;}=true;}
