...
{
    readonly string dataDir=Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"ConvoyHub");
    ...
    public MainWindow()
    {
        ...
        ApiUrlBox.Text=settings.ApiUrl;ApiKeyBox.Password=settings.ApiKey;IntervalBox.Text=settings.SendIntervalMs.ToString();AutoStartCheck.IsChecked=settings.AutoStart;MinimizeTrayCheck.IsChecked=settings.MinimizeToTray;AutoSyncCheck.IsChecked=settings.AutoSync;AutoTripCheck.IsChecked=settings.AutoTrip;
    ...
    async Task ProcessPacket(TelemetryPacket p)
    {
        ...
        var payload=new{tripId=trip.Id,jobKey,hasJob,@event=p.Event??"telemetry",vtcId=SelectedVtc(),userId=string.IsNullOrWhiteSpace(settings.UserId)?(string.IsNullOrWhiteSpace(settings.DriverName)?"DesktopDriver":settings.DriverName):settings.UserId,game=p.Game is "ATS"?"ATS":"ETS2",latitude=MapLatitude(p),longitude=MapLongitude(p),gameX=p.WorldX,gameY=p.WorldY,gameZ=p.WorldZ,heading=p.Heading,speedKph=p.SpeedKph,rpm=p.Rpm,fuelLiters=p.Fuel,odometerKm=p.Odometer,truckDamage=p.TruckDamage,trailerDamage=p.TrailerDamage,cargo=p.Cargo,cargoMass=p.CargoMass,sourceCity=p.SourceCity,sourceCompany=p.SourceCompany,destinationCity=p.DestinationCity,destinationCompany=p.DestinationCompany,plannedDistanceKm=p.PlannedDistanceKm,gameIncomeCents=p.GameIncomeCents,truck=p.Truck,server=p.Server,recordedAt=DateTime.UtcNow};
        var json=JsonSerializer.Serialize(payload);var result=await Send(json);
        if(result.Ok){syncedCount++;if(!string.IsNullOrWhiteSpace(result.TripId)){trip.Id=result.TripId;trip.JobKey=jobKey;}tripPoints=Math.Max(tripPoints,result.PointsTotal);if(result.Lifecycle=="pending_driver"){await Dispatcher.InvokeAsync(CompleteTrip);await Dispatcher.InvokeAsync(async()=>{if(MessageBox.Show("Der Auftrag wurde geliefert. Möchtest du die Abrechnung jetzt bestätigen und zur Lohnzahlung einreichen?","ConvoyHub Abrechnung",MessageBoxButton.YesNo,MessageBoxImage.Question)==MessageBoxResult.Yes)await ConfirmInvoiceAsync();});}else if(result.Lifecycle=="cancelled")await Dispatcher.InvokeAsync(AbortTrip);else if(result.Lifecycle=="interrupted"){trip.Status="Unterbrochen";trip.Active=true;}Save(RecoveryPath,trip);Dispatcher.Invoke(()=>{MetricSynced.Text=syncedCount.ToString();PointCounterText.Text=$"{tripPoints} PUNKTE";UpdateTripUi();});}
        else{File.AppendAllText(QueuePath,json+Environment.NewLine);Dispatcher.Invoke(UpdateQueueCount);}
    }
    async Task ConfirmInvoiceAsync(){if(string.IsNullOrWhiteSpace(trip.Id))return;try{var json=JsonSerializer.Serialize(new{action="confirmTrip",tripId=trip.Id});var res=await http.PostAsync(Api("/api/v1/payroll"),new StringContent(json,Encoding.UTF8,"application/json"));if(!res.IsSuccessStatusCode){var error=await res.Content.ReadAsStringAsync();MessageBox.Show("Die Abrechnung konnte nicht bestätigt werden. "+error,"ConvoyHub",MessageBoxButton.OK,MessageBoxImage.Warning);return;}trip.Status="Abrechnung bestätigt – wartet auf Lohnfreigabe";Save(RecoveryPath,trip);UpdateTripUi();MessageBox.Show("Abrechnung bestätigt. Sie wartet jetzt im Gründer-/Lohnbüro auf Freigabe.","ConvoyHub",MessageBoxButton.OK,MessageBoxImage.Information);Log("Abrechnung zur Lohnfreigabe eingereicht: "+trip.Id);}catch(Exception ex){Log("Abrechnungsbestätigung fehlgeschlagen: "+ex.Message);MessageBox.Show("Server nicht erreichbar. Bitte später im Fahrtenbuch bestätigen.","ConvoyHub",MessageBoxButton.OK,MessageBoxImage.Warning);}}
    ...
    void InstallPlugin(string game){var path=game=="ETS2"?etsPath:atsPath;var target=PluginTarget(path);var source=PluginSource();if(path is null||target is null){MessageBox.Show("Die Spielinstallation wurde nicht gefunden.","ConvoyHub",MessageBoxButton.OK,MessageBoxImage.Warning);return;}if(!File.Exists(source)){MessageBox.Show("Die native Plugin-DLL ist noch nicht im Client-Paket vorhanden. Baue das Projekt ConvoyHub.ScsPlugin zuerst.","ConvoyHub",MessageBoxButton.OK,MessageBoxImage.Warning);return;}Directory.CreateDirectory(Path.GetDirectoryName(target)!);File.Copy(source,target,true);Log($"Plugin für {game} installiert: {target}");UpdatePluginState();MessageBox.Show("Plugin installiert. Starte das Spiel neu und bestätige den SCS-SDK-Hinweis.","ConvoyHub",MessageBoxButton.OK,MessageBoxImage.Information);}
    static void StartUri(string uri){Process.Start(new ProcessStartInfo(uri){UseShellExecute=true});}
    string SelectedVtc()=>Dispatcher.CheckAccess()?((VtcCombo.SelectedItem as ComboBoxItem)?.Tag?.ToString())??"vtc-ngl":Dispatcher.Invoke(SelectedVtc);

    async void Login_Click(object sender,RoutedEventArgs e){try{var json=JsonSerializer.Serialize(new{email=EmailBox.Text.Trim(),password=PasswordInput.Password});var res=await http.PostAsync(Api("/api/auth/login"),new StringContent(json,Encoding.UTF8,"application/json"));if(!res.IsSuccessStatusCode){AccountState.Text="Anmeldung fehlgeschlagen";return;}var doc=JsonDocument.Parse(await res.Content.ReadAsStringAsync());var user=doc.RootElement.GetProperty("user");ApplyAuthenticatedUser(user);Log("Konto mit E-Mail und Passwort angemeldet");}catch(Exception ex){AccountState.Text="Server nicht erreichbar";Log("Login fehlgeschlagen: "+ex.Message);}}
    async void OAuthLogin_Click(object sender,RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            if(!response.IsSuccessStatusCode){AccountState.Text=$"{provider} ist auf dem Server noch nicht konfiguriert";return;}
    ...
    void ApplyAuthenticatedUser(JsonElement user){settings.UserId=user.GetProperty("id").GetString()??"";settings.DriverName=user.TryGetProperty("displayName",out var name)?name.GetString()??"DesktopDriver":"DesktopDriver";AccountState.Text=$"Angemeldet als {settings.DriverName}";Save(SettingsPath,settings);}
    async Task CheckForUpdatesAsync(bool manual=false)
    {
        try
        {
            ...
            var asset=release.RootElement.GetProperty("assets").EnumerateArray().FirstOrDefault(x=>(x.GetProperty("name").GetString()??"").Contains("Desktop-Client",StringComparison.OrdinalIgnoreCase)&&(x.GetProperty("name").GetString()??"").EndsWith(".zip",StringComparison.OrdinalIgnoreCase));
    ...
    async Task InstallUpdateAsync(Uri downloadUri,string? expectedDigest,Version version)
    {
        ...
        var extract=Path.Combine(root,"package");
        ...
        if(!File.Exists(Path.Combine(extract,"VTCTruckHub.Client.exe"))||!File.Exists(Path.Combine(extract,"plugins","convoyhub_scs.dll")))throw new InvalidDataException("Das Releasepaket enthält nicht alle Pflichtdateien.");
        var script=Path.Combine(root,"apply-update.ps1");
        var targetDir=AppContext.BaseDirectory.TrimEnd(Path.DirectorySeparatorChar);
        var exeName="VTCTruckHub.Client.exe";
        ...
        Process.Start(new ProcessStartInfo("powershell.exe",$"-NoProfile -ExecutionPolicy Bypass -File \"{script}\""){UseShellExecute=true,WindowStyle=ProcessWindowStyle.Hidden});
        forceClose=true;Log($"Update {version} heruntergeladen und zur Installation vorbereitet.");System.Windows.Application.Current.Shutdown();
    }
    ...
    void SaveSettings_Click(object sender,RoutedEventArgs e){settings.ApiUrl=ApiUrlBox.Text.Trim();settings.ApiKey=ApiKeyBox.Password;settings.SendIntervalMs=int.TryParse(IntervalBox.Text,out var ms)?Math.Clamp(ms,250,10000):1000;settings.AutoStart=AutoStartCheck.IsChecked==true;settings.MinimizeToTray=MinimizeTrayCheck.IsChecked==true;settings.AutoSync=AutoSyncCheck.IsChecked==true;settings.AutoTrip=AutoTripCheck.IsChecked==true;Save(SettingsPath,settings);using var key=Registry.CurrentUser.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run",true);if(settings.AutoStart)key?.SetValue("ConvoyHub",$"\"{Environment.ProcessPath}\"");else key?.DeleteValue("ConvoyHub",false);Log("Einstellungen gespeichert");MessageBox.Show("Einstellungen gespeichert.","ConvoyHub");}
    void ShowFromTray(){Show();WindowState=WindowState.Normal;Activate();}void Window_Closing(object? sender,System.ComponentModel.CancelEventArgs e){if(!forceClose&&settings.MinimizeToTray){e.Cancel=true;Hide();tray.ShowBalloonTip(1500,"ConvoyHub","Der Client läuft im Infobereich weiter.",Forms.ToolTipIcon.Info);return;}shutdown.Cancel();udp?.Dispose();tray.Visible=false;tray.Dispose();}
}

public sealed class ClientSettings{public string ApiUrl{get;set;}="http://localhost:3000";public string ApiKey{get;set;}="demo-client-key";public string UserId{get;set;}="";public string DriverName{get;set;}="DesktopDriver";public int SendIntervalMs{get;set;}=1000;public bool AutoStart{get;set;}public bool MinimizeToTray{get;set;}=true;public bool AutoSync{get;set;}=true;public bool AutoTrip{get;set;}=true;}
...
public sealed class TelemetryPacket{public string Game{get;set;}="ETS2";public string? Event{get;set;}public string? JobKey{get;set;}public double WorldX{get;set;}public double WorldY{get;set;}public double WorldZ{get;set;}public double Heading{get;set;}public double SpeedKph{get;set;}public double Rpm{get;set;}public int Gear{get;set;}public double Fuel{get;set;}public double FuelAverage{get;set;}public bool CruiseControl{get;set;}public bool EngineEnabled{get;set;}public double NavigationDistance{get;set;}public double TruckDamage{get;set;}public double TrailerDamage{get;set;}public int GameTime{get;set;}public double Odometer{get;set;}public string? Truck{get;set;}public string? Cargo{get;set;}public double CargoMass{get;set;}public string? SourceCity{get;set;}public string? SourceCompany{get;set;}public string? DestinationCity{get;set;}public string? DestinationCompany{get;set;}public double PlannedDistanceKm{get;set;}public int GameIncomeCents{get;set;}public string? Server{get;set;}}
