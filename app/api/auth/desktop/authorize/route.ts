import { ensureDatabase, isSecureRequest, platformEnv } from "@/lib/platform";
...
export async function GET(request: Request) {
  ...
  return new Response(null, { status: 302, headers: { Location: `/api/auth/${provider}/start`, "Set-Cookie": `desktop_auth=${encodeURIComponent(token)}; Path=/api/auth; HttpOnly; SameSite=Lax; Max-Age=600${isSecureRequest(request) ? "; Secure" : ""}` } });
}
