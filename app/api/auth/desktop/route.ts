...
export async function GET(request: Request) {
  ...
  const issued=await issuePersonalClientKey(row.id);
  ...
  return Response.json({ status: "approved", user: { id: row.id, displayName: row.displayName, email: row.email },memberships:boundMemberships,clientKey:issued.key,apiBase:publicRequestOrigin(request) });
}
