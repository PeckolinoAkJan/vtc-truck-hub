import {apiError,audit,ensureDatabase,getSessionUser,platformEnv,randomId,requireFounder,requireVtcPermission} from "@/lib/platform";

const imagePurposes=new Set(["avatar","company_logo","company_header","gallery","event_image","vehicle_image","discord_image","platform_image"]);
...
export async function POST(request:Request){
  ...
  let manager=false;if(purpose.startsWith("company_")){if(!vtcId)return apiError("Spedition fehlt");manager=Boolean(await requireVtcPermission(request,vtcId,"manage_settings"));if(!manager)return apiError("Einstellungsrecht erforderlich",403)}else if(purpose==="platform_image"){manager=Boolean(await requireFounder(request));if(!manager)return apiError("Nur das Gründerkonto darf Plattformbilder hochladen",403)}else if(purpose==="discord_image"){manager=Boolean(vtcId?(await requireVtcPermission(request,vtcId,"manage_discord")??await requireVtcPermission(request,vtcId,"manage_settings")):await requireFounder(request));if(!manager)return apiError("Discord-Verwaltungsrecht erforderlich",403)}else if(vtcId){manager=Boolean(await requireVtcPermission(request,vtcId,"manage_gallery"));const member=await platformEnv().DB.prepare(`SELECT id FROM memberships WHERE vtc_id=? AND user_id=? AND status IN ('active','probation')`).bind(vtcId,user.id).first();if(!manager&&!member)return apiError("Keine Mitgliedschaft in dieser Spedition",403)}
