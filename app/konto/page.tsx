        ...
        {user ? (
          <div className="account-success">
            ...
            <section className="client-connect-box">
              <div><span>DESKTOP-CLIENT</span><h2>Telemetrie sicher verbinden</h2><p>Jeder Fahrer erhält einen festen persönlichen Schlüssel. Der Server akzeptiert ihn ausschließlich für Speditionen, in denen das Konto aktuell Mitglied ist.</p></div>
              {clientAccess.length?clientAccess.map((entry:any)=><article key={entry.id}><div><b>{entry.tag} · {entry.name}</b><small>{entry.roleName||"Mitglied"} · {entry.keyPrefix?`Persönlicher Schlüssel ${entry.keyPrefix}…`:"Kontoschlüssel wird beim Client-Login angelegt"}</small></div><button type="button" onClick={()=>createClientKey(entry.id)}>Schlüssel anzeigen</button></article>):<p className="client-empty">Dein Kontoschlüssel ist bereits vorbereitet. Sobald du einer Spedition beitrittst, wird sie im Desktop-Client automatisch ergänzt.</p>}
