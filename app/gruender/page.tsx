...
const tabs = ["Übersicht", "Wirtschaft & Punkte", "Abrechnungen", "Discord-Bot", "Support-Tickets", "Benutzer & Speditionen", "Moderation", "System & Features", "Clientversionen & Backups", "Audit & Sicherheit"];
...
export default function Founder() {
  ...
  return <main className="founder-shell" aria-busy={busy}>
    ...
    <section className="founder-main">
      ...
      {tab === "Abrechnungen" && <div className="admin-list"><h2>Zur Lohnzahlung</h2>{data.payrolls.length ? data.payrolls.map((payroll) => <article key={payroll.id}><div><strong>{payroll.driver ?? payroll.id}</strong><span>{payroll.period} · {payroll.status}</span></div><b>{(payroll.netCents / 100).toLocaleString("de-DE", { style: "currency", currency: "EUR" }).replace("€", "V€")}</b>{payroll.status === "submitted" && <button disabled={busy} onClick={() => run({ action: "approvePayroll", payrollId: payroll.id }, "Abrechnung bestätigt und ausgezahlt.")}>Bestätigen & auszahlen</button>}</article>) : <p>Keine Abrechnung wartet auf Freigabe.</p>}</div>}
